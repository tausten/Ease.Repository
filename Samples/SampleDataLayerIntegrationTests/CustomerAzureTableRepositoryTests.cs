...
{
    public class CustomerAzureTableRepositoryTests
        : AzureTableRepositoryTests<SampleAzureTableMainRepositoryContext, CustomerAzureTableEntity, CustomerAzureTableRepository>
    {
        ...
        protected override void ModifyEntity(CustomerAzureTableEntity entityToModify)
        {
        ...
        protected override void AssertEntitiesAreEquivalent(CustomerAzureTableEntity result, CustomerAzureTableEntity reference)
        {
            result.CurrentState().Should().BeEquivalentTo(reference.CurrentState(), options => options
                .Excluding(x => x.Timestamp)
                .Using<DateTime>(ctx => ctx.Subject.ToUniversalTime().ToShortDateString()
                        .Should().Be(ctx.Expectation.ToUniversalTime().ToShortDateString()))
                    .When(info => info.SelectedMemberPath.EndsWith(nameof(CustomerAzureTableEntity.Birthday)))
                );
        }

        protected override ITableEntity NewSimpleKeyFromEntity(CustomerAzureTableEntity entity)
        {
